...
class Main_salary_employee_LoansController extends Controller
{
    public function index()
    {
        ...
        return view('admin.Main_salary_employee_loans.index', compact('Finance_cin_periods'));
    }
    ...
    public function show($finance_cin_periods_id){
        ...
        $data = get_cols_where_paginate(new Main_salary_employee_loans(), array('*'), array('com_code' => $com_code,'finance_cin_periods_id'=>$finance_cin_periods_id),'id','DESC',PAGINATION_COUNTER);
        if(!empty($data)){
        ...
        return view('admin.Main_salary_employee_loans.show', ['data'=>$data,'finance_cin_periods_data'=>$finance_cin_periods_data,'employees'=>$employees,'employees_search'=>$employees_search]);
    ...
    public function checkExsistsBefor(Request $request){
        ...
        if($request->ajax()){
            $checkExsistsBeforCounter=get_count_where(new Main_salary_employee_loans(),array('com_code'=>$com_code,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'employees_code'=>$request->employees_code));
    ...
    public function ajax_search(Request $request) {
        if($request->ajax()){
            ...
            $com_code = auth()->user()->com_code;
            $data = Main_salary_employee_loans::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where('finance_cin_periods_id','=',$the_finance_cin_periods_id)->where('com_code','=',$com_code)->orderBy('id','DESC')->paginate(PAGINATION_COUNTER);
    ...
    public function load_edit_row(Request $request) {
        if($request->ajax()){
            ...
            return view('admin.Main_salary_employee_loans.load_edit_row',['finance_cin_periods_data'=>$finance_cin_periods_data,'main_salary_employee_data'=>$main_salary_employee_data,'data_row'=>$data_row,'employees'=>$employees]);
        }
    ...
    public function print_search(Request $request) {
            ...
            $other['value_sum']=0;
            $other['total_sum']=0;
            $data = Main_salary_employee_loans::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where('finance_cin_periods_id','=',$the_finance_cin_periods_id)->where('com_code','=',$com_code)->orderBy('id','DESC')->get();
            ...
            return view('admin.Main_salary_employee_loans.print_search', ['data'=>$data,'finance_cin_periods_data'=>$finance_cin_periods_data,'systemData'=>$systemData,'other'=>$other]);
