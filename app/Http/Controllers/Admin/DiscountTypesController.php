...
class DiscountTypesController extends Controller
{
    public function index()
    {
        ...
        $Discount_Types = get_cols_where_paginate(new Discount_Type(), array('*'), array('com_code' => $com_code), 'id', 'DESC', PAGINATION_COUNTER);
        return view('admin.DiscountType.index', compact('Discount_Types'));
    }
    ...
    {
        return view('admin.DiscountType.create');
    }
    ...
    public function store(DiscountTypeRequest $request)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('DiscountType.index')->with('success', 'تم اضافة النوع بنجاح');
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('DiscountType.index')->with('error', 'عفواً غير قادر الوصول الى البيانات');
        }
        return view('admin.DiscountType.edit', compact('data'));
    }
    ...
    public function update(DiscountTypeRequest $request, $id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $checkExsists = Discount_Type::select('id')->where('com_code', $com_code)->where('id', '!=', $id)->where('name', $request->name)->first();
            if (!empty($checkExsists)) {
                return redirect()->back()->with('error', 'عفواً هذه النوع مسجل من قبل')->withInput();
            }
            ...
            DB::commit();
            return redirect()->route('DiscountType.index')->with('success', 'تم تعديل النوع بنجاح');
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with('error', 'عفواً حدث خطأ')->withInput();
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('DiscountType.index')->with('success', 'تم حذف النوع بنجاح');
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with('error', 'عفواً حدث خطأ')->withInput();
        }
