...
class Main_salary_employee_AllowancesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.Main_salary_employee_allowances.index', compact('Finance_cin_periods'));
    }
    ...
    public function show($finance_cin_periods_id){
        ...
        if(empty($finance_cin_periods_data)){
            return redirect()->route('MainSalaryAllowances.index')->with('error','عفواً غير قادر للوصول الى البينات المطلوبة');
        }

        $data = get_cols_where_paginate(new Main_salary_employee_allowances(), array('*'), array('com_code' => $com_code,'finance_cin_periods_id'=>$finance_cin_periods_id),'id','DESC',PAGINATION_COUNTER);
        if(!empty($data)){
            foreach($data as $info){
                $info->emp_name=get_field_value(new Employee(),'emp_name',array('com_code'=>$com_code,'employees_code'=>$info->employees_code));
                $info->allowances_name=get_field_value(new Allowance(),'name',array('com_code'=>$com_code,'id'=>$info->allowances_id));
            }
        }
        ...
        $allowance=get_cols_where(new Allowance(),array("id","name"),array("com_code"=>$com_code));
        ...
        return view('admin.Main_salary_employee_allowances.show', ['data'=>$data,'finance_cin_periods_data'=>$finance_cin_periods_data,'employees'=>$employees,'employees_search'=>$employees_search,'allowance'=>$allowance]);
    ...
    public function checkExsistsBefor(Request $request){
        ...
        if($request->ajax()){
            $checkExsistsBeforCounter=get_count_where(new Main_salary_employee_allowances(),array('com_code'=>$com_code,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'employees_code'=>$request->employees_code));
        ...
        if($request->ajax()){
            ...
            if(!empty($finance_cin_periods_data) && !empty($main_salary_employee_data)){
                ...
                $dataToInsert=[
                    ...
                    'allowances_id'=>$request->allowance_add ,
                    'total'=>$request->total ,
                    'notes'=>$request->notes ,
            ...
            $flag=insert(new Main_salary_employee_allowances(),$dataToInsert);
            if(!empty($flag)){
    ...
    public function ajax_search(Request $request) {
        if($request->ajax()){
            $employees_code=$request->employees_code;
            $allowance_search=$request->allowance_search;
            ...
            $com_code = auth()->user()->com_code;
            $data = Main_salary_employee_allowances::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cin_periods_id','=',$the_finance_cin_periods_id)->where('com_code','=',$com_code)->orderBy('id','DESC')->paginate(PAGINATION_COUNTER);
    ...
    public function delete_row(Request $request) {
        ...
        if($request->ajax()){
            ...
            $data_row=get_cols_where_row(new Main_salary_employee_allowances(),array("id"),array('com_code'=>$com_code,'id'=>$request->id,'is_archived'=>0,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
            if(!empty($finance_cin_periods_data) and !empty($data_row) and !empty($main_salary_employee_data)){

                $flag=destroy(new Main_salary_employee_allowances(),array('com_code'=>$com_code,'id'=>$request->id,'is_archived'=>0,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
                if(!empty($flag)){
    ...
    public function load_edit_row(Request $request) {
        if($request->ajax()){
            ...
            $finance_cin_periods_data=get_cols_where_row(new Finance_cin_periods(),array("id"),array('com_code'=>$com_code,'id'=>$request->the_finance_cin_periods_id,'is_open'=>1));
            $main_salary_employee_data=get_cols_where_row(new Main_salary_employee(),array("id"),array('com_code'=>$com_code,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'id'=>$request->main_salary_employee_id,'is_archived'=>0));
            $data_row=get_cols_where_row(new Main_salary_employee_allowances(),array("*"),array('com_code'=>$com_code,'id'=>$request->id,'is_archived'=>0,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
            ...
            return view('admin.Main_salary_employee_allowances.load_edit_row',['finance_cin_periods_data'=>$finance_cin_periods_data,'main_salary_employee_data'=>$main_salary_employee_data,'data_row'=>$data_row,'employees'=>$employees,'allowance'=>$allowance]);
        }
        ...
        if($request->ajax()){
            ...
            if(!empty($finance_cin_periods_data) && !empty($main_salary_employee_data) && !empty($data_row)){
                ...
                $dataToUdate=[
                    ...
                    'employees_code'=>$request->employees_code ,
                    'day_price'=>$request->day_price ,
                    'allowances_id'=>$request->allowance_edit ,
                    'total'=>$request->total ,
                    'notes'=>$request->notes ,
            ...
            $flag=update(new Main_salary_employee_allowances(),$dataToUdate,array('com_code'=>$com_code,'id'=>$request->id,'is_archived'=>0,'finance_cin_periods_id'=>$request->the_finance_cin_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
            if(!empty($flag)){
                $this->Recalculate_main_salary_employee($request->main_salary_employee_id);
            } 
            DB::commit();
    ...
    public function print_search(Request $request) {
            ...
            $other['value_sum']=0;
            $other['total_sum']=0;
            $data = Main_salary_employee_allowances::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cin_periods_id','=',$the_finance_cin_periods_id)->where('com_code','=',$com_code)->orderBy('id','DESC')->get();
            ...
            return view('admin.Main_salary_employee_allowances.print_search', ['data'=>$data,'finance_cin_periods_data'=>$finance_cin_periods_data,'systemData'=>$systemData,'other'=>$other]);
