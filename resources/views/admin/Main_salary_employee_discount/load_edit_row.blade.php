    ...
    <div class="row">
        ...
        <div class="col-md-3 ">
            <div class="form-group">
                ...
                <select name="discount_types_edit" id="discount_types_edit" class="form-control">
                    <option disabled selected value="">اختر النوع</option>
                    @if (@isset($discount_type) && !@empty($discount_type))
                    @foreach ($discount_type as $info)
                        <option @if ($data_row['discount_types_id']==$info->id) selected @endif value="{{ $info->id }}"> {{ $info->name }}
