    ...
    <div class="row">
        ...
        <div class="col-md-3 ">
            <div class="form-group">
                ...
                <select name="allowance_edit" id="allowance_edit" class="form-control">
                    <option disabled selected value="">اختر النوع</option>
                    @if (@isset($allowance) && !@empty($allowance))
                    @foreach ($allowance as $info)
                        <option @if ($data_row['allowances_id']==$info->id) selected @endif value="{{ $info->id }}"> {{ $info->name }}
        ...
        <div class="col-md-3">
            <div class="form-group text-left">
                <button style="margin-top: 33px" id="do_edit_now" class="btn btn-sm btn-danger" type="submit" data-id="{{ $data_row['id'] }}"
                    ...
                    name="submit">تعديل البدل</button>
            </div>
        </div>
