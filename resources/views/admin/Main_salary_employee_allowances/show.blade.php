...
@section('contentheaderactivelink')
    <a href="{{ route('MainSalaryAllowances.index') }}">البدلات المتغيرة</a>
@endsection
...
@section('content')
    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-header">
                <h3 class="card-title card_title_center">بيانات البدلات المتغيرة للشهر المالي
            ...
            <form action="{{ route('MainSalaryAllowances.print_search') }}" method="POST" target="_blank">
                @csrf
                ...
                <div class="row" style="padding: 5px">
                    ...
                    <div class="col-md-3 ">
                        <div class="form-group">
                            ...
                            <select name="allowance_search" id="allowance_search" class="form-control select2">
                                <option value="all">بحث بالكل</option>
                                @if (@isset($allowance) && !@empty($allowance))
                                    @foreach ($allowance as $info)
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                <div class="modal-header">
                    <h4 class="modal-title">اضافة البدلات المتغيرة للموظفين بالشهر المالي</h4>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 ">
                            <div class="form-group">
                                ...
                                <select name="allowance_add" id="allowance_add" class="form-control select2">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="form-group text-center">
                                <button style="margin-top: 33px" id="do_add_now" class="btn btn-sm btn-danger"
                                    type="submit" name="submit">اضف البدل</button>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="EditModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                <div class="modal-header">
                    <h4 class="modal-title">تعديل البدلات المتغيرة للموظفين بالشهر المالي</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#do_add_now', function(e) {
                ...
                var allowance_add = $("#allowance_add").val();
                if (allowance_add == "") {
                    alert("من فضلك اختر نوع البدل");
                    $("#allowance_add").focus();
                    return false;
                }
                ...
                if (total_add == "") {
                    alert("من فضلك ادخل اجمالي البدل");
                    $("#total_add").focus();
                    return false;
                }
                ...
                jQuery.ajax({
                    url: '{{ route('MainSalaryAllowances.checkExsistsBefor') }}',
                    type: 'post',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    success: function(data) {
                        if (data == 'exsists_befor') {
                            var result = confirm(
                                "يوجد سجل البدل مسجلة سابقة للموظف هل تريد الاستمرار"
                            );
                            if (result == true) {
                                var flagResult = true;
                            } else {
                        ...
                        if (flagResult == true) {
                            ...
                            jQuery.ajax({
                                url: '{{ route('MainSalaryAllowances.store') }}',
                                type: 'post',
                                dataType: 'html',
                                cache: false,
                                data: {
        ...
        $(document).on('change', '#allowance_search', function(e) {
            ajax_search();
        ...
        function ajax_search() {
            ...
            var allowance_search = $("#allowance_search").val();
            var is_archived = $("#is_archived_search").val();
            var the_finance_cin_periods_id = $('#the_finance_cin_periods_id').val();

            jQuery.ajax({
                url: '{{ route('MainSalaryAllowances.ajaxSearch') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '.delete_this_row', function(e) {
            ...
            $("#backup_freeze_modal").modal("show");
            jQuery.ajax({
                url: '{{ route('MainSalaryAllowances.delete_row') }}',
                type: 'post',
                dataType: 'json',
                cache: false,
                data: {
        ...
        $(document).on('click', '.load_edit_this_row', function(e) {
            ...
            var main_salary_employee_id = $(this).data("main_sal_id");
            jQuery.ajax({
                url: '{{ route('MainSalaryAllowances.load_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '#do_edit_now', function(e) {
            ...
            var allowance_edit = $("#allowance_edit").val();
            if (allowance_edit == "") {
                alert("من فضلك اختر نوع البدل");
                $("#allowance_edit").focus();
                return false;
            }
            ...
            jQuery.ajax({
                url: '{{ route('MainSalaryAllowances.do_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
