...
@section('contentheaderactivelink')
    <a href="{{ route('MainSalaryLoans.index') }}">السلف الشهرية</a>
@endsection
...
@section('content')
    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-header">
                <h3 class="card-title card_title_center">بيانات السلف الشهرية للشهر المالي
            ...
            <form action="{{ route('MainSalaryLoans.print_search') }}" method="POST" target="_blank">
                @csrf
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                <div class="modal-header">
                    <h4 class="modal-title">اضافة سلفة شهرية للموظفين بالشهر المالي</h4>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="form-group text-center">
                                <button style="margin-top: 33px" id="do_add_now" class="btn btn-sm btn-danger"
                                    type="submit" name="submit">اضف السلفة</button>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="EditModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                <div class="modal-header">
                    <h4 class="modal-title">تعديل سلفة الشهرية للموظفين بالشهر المالي</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#do_add_now', function(e) {
                ...
                jQuery.ajax({
                    url: '{{ route('MainSalaryLoans.checkExsistsBefor') }}',
                    type: 'post',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    success: function(data) {
                        if (data == 'exsists_befor') {
                            var result = confirm(
                                "يوجد سجل السلفة مسجلة سابقة للموظف هل تريد الاستمرار"
                            );
                            if (result == true) {
                                var flagResult = true;
                            } else {
                        ...
                        if (flagResult == true) {
                            ...
                            jQuery.ajax({
                                url: '{{ route('MainSalaryLoans.store') }}',
                                type: 'post',
                                dataType: 'html',
                                cache: false,
                                data: {
        ...
        function ajax_search() {
            ...
            jQuery.ajax({
                url: '{{ route('MainSalaryLoans.ajaxSearch') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '.load_edit_this_row', function(e) {
            ...
            var main_salary_employee_id = $(this).data("main_sal_id");
            jQuery.ajax({
                url: '{{ route('MainSalaryLoans.load_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '#do_edit_now', function(e) {
            ...
            jQuery.ajax({
                url: '{{ route('MainSalaryLoans.do_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
